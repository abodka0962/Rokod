...
$items = [];
if ($id) {
    $result = $conn->query("SELECT * FROM sales WHERE id=$id LIMIT 1");
    $row = $result ? $result->fetch_assoc() : null;
    $itemsResult = $conn->query("SELECT * FROM sales_items WHERE sale_id=$id");
    if ($itemsResult) {
        while($item = $itemsResult->fetch_assoc()) $items[] = $item;
    }
}

function sales_invoice_number($id) {
    return 'SAL-' . str_pad($id, 6, '0', STR_PAD_LEFT);
}
...
<head>
<meta charset="UTF-8">
<title>فاتورة مبيعات | <?= $company_name ?></title>
<style>
...
.invoice-box {
    ...
    border-bottom: 7px solid #43a047;
}
.header-brand {
    text-align: center;
    margin-bottom: 15px;
...
.header { text-align: center; color: #388e3c; font-size: 1.5em; font-weight: bold; margin-bottom: 25px;}
...
.table th { background: #e8f5e9; color: #2e7d32; font-weight: bold;}
.total-box { margin-top:20px; padding: 13px 17px; background: #f3f5fa; border-radius: 10px; border: 1px solid #e3e8fd; font-size:1.13em;}
.total-box .row { display: flex; justify-content: space-between; margin-bottom: 7px;}
.total-box .final { color: #388e3c; font-size: 1.19em; font-weight: bold;}
@media print {
    body { background: #fff; }
    .invoice-box { box-shadow: none; border-top: 4px solid #43a047; border-bottom: 4px solid #43a047;}
}
</style>
</head>
<body>
<div class="invoice-box">
    ...
    <?php if ($row): ?>
        <div class="data-row">
            ...
            <div class="value"><?= sales_invoice_number($row['id']) ?></div>
        </div>
        <div class="data-row">
        ...
        <div class="data-row">
            ...
            <div class="value"><?= htmlspecialchars($row['customer_name']) ?></div>
        </div>
        <table class="table">
            <tr>
                <th>المنتج</th>
